...
export class FollowService {
	...
	async follow(user: User, channelId: string) {
		...
		if (channel.id === user.id) {
			throw new ConflictException('Not possible subscribe yourself');
		}
		...
		if (existingFollow) {
			throw new ConflictException(
				"You can't subscribe same channel 2 times",
			);
		}
	...
	async unfollow(user: User, channelId: string) {
		...
		if (channel.id === user.id) {
			throw new ConflictException('Not possible unsubscribe yourself');
		}
		...
		if (!existingFollow) {
			throw new ConflictException('You not subscribed this channel');
		}
